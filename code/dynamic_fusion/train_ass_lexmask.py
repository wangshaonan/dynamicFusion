...
class lasagne_average_layer(lasagne.layers.MergeLayer):
    def __init__(self, incoming, num_units=428, W=lasagne.init.Normal(), b=lasagne.init.Normal(), **kwargs):
        super(lasagne_average_layer, self).__init__(incoming, **kwargs)
	self.W = self.add_param(W, (428,428))
	self.b = self.add_param(b, (428,))

    def get_output_for(self, inputs, **kwargs):
        emb = inputs[0]
        emb = emb.sum(axis=1)
	mask = lasagne.nonlinearities.tanh(T.dot(emb, self.W)+self.b)
	emb = emb*mask
        return emb
